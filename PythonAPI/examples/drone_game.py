...
class Angular_rate:
    ...
    def imu_callback(self, imu_data):
        ...
        self.roll_rate = angular_velocity.x 
        self.pitch_rate = angular_velocity.y 
        self.yaw_rate = angular_velocity.z 
...
class controller:
    def __init__(self):
        ...
        self.yaw_rate_pid = PID(1.0, 0, 2, setpoint=0, output_limits=(-1, 1))
    ...
    def pid_calculate(self,vehicle_transform,vehicle_velocity,vehicle_angular_rate):
        ...
        yaw_rate_output = self.yaw_rate_pid(vehicle_angular_rate.yaw_rate)
...
def main():
    ...
    vehicle_angular_rate=Angular_rate(imu_sensor)
    imu_sensor.listen(lambda imu_data: vehicle_angular_rate.imu_callback(imu_data))
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    ctl.handle_keys(event.type,event.key)
            ...
            front_left,front_right,rear_left,rear_right=ctl.pid_calculate(vehicle_transform,vehicle_velocity,vehicle_angular_rate)
